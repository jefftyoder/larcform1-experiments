# Phase 1: Larcform1 SCM coupled to Pithan-correct slab sea ice via ClimaCoupler.
#
# Includes the custom `larcform1_ice` component (registered via Interfacer
# Val-dispatch — no ClimaCoupler fork), merges the validated standalone atmos
# config with a coupler overlay (configs/lf1_larcform1_ice_overlay.yml),
...
#   julia --project experiments/sea-ice/run_larcform1_ice.jl
#   julia --project experiments/sea-ice/run_larcform1_ice.jl --dry-run   # setup + config check only
...
const OVERLAY_CONFIG = joinpath(@__DIR__, "configs/lf1_larcform1_ice_overlay.yml")
...
merged_path = joinpath(@__DIR__, "configs/generated_lf1_larcform1_ice.yml")
