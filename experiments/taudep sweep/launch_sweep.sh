...
tmux new-session -d -s lf1sweep \
  "julia +1.12 -t 1 --startup-file=no --project=ClimaAtmos.jl/.buildkite 'experiments/taudep sweep/run_sweep.jl' sweep --z_elem $Z_ELEM $* 2>&1 | tee output/lf1e-taudep-1/sweep_$(date +%Y%m%d_%H%M%S).log"
